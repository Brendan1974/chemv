...
class ActivityStatement(models.AbstractModel):
    ...
    def _initial_balance_sql_q1(self, partners, date_start, account_type):
        ...
        return str(
            self._cr.mogrify(
                """
            SELECT l.partner_parent_id AS partner_id, l.currency_id, l.company_id, l.id,
            ...
            GROUP BY l.partner_parent_id, l.currency_id, l.company_id, l.balance, l.id
        """,
                locals(),
            ),
            "utf-8",
    ...
    def _display_activity_lines_sql_q1(
        ...
        return str(
            self._cr.mogrify(
                """
            SELECT m.name AS move_id, l.partner_parent_id AS partner_id, l.date,
    ...
    def _display_activity_reconciled_lines_sql_q2(self, sub, date_end):
        return str(
            self._cr.mogrify(
                f"""
            SELECT l.id as rel_id, m.name AS move_id, l.partner_parent_id AS partner_id, l.date, l.name,
    ...
    def _get_report_values(self, docids, data=None):
        ...
        # chemv portal hook: when invoked from a website controller (website_id
        # set in context) that supplies its own date range, honour it.
        if (
            self._context.get("website_id")
            and self._context.get("date_start")
            and self._context.get("date_end")
        ):
